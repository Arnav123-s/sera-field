...
def main():
    ...
    decision = read(ROOT / 'reports' / NAME / 'QUALIFICATION.json')
    owner, selected, payload = load_native(STUDY / 'credited')
    target = ROOT / 'checkpoints' / NAME; revisions = target / 'revisions'; revisions.mkdir(parents=True, exist_ok=True)
    if not (target / 'SELECTION.json').exists():
        ...
        write_json(target / 'MANIFEST.json', {'inference': packaged, 'source_training': selected,
            ...
            'qualification': 'reports/'+NAME+'/QUALIFICATION.json',
    ...
    write_json(ROOT / 'reports' / NAME / 'PACKAGE.json', {'exact_tensors': all(checks.values()),
