"""Complete the frozen lexical repair and four independent five-use replays."""
import json
import os
...
def main():
    ...
    root = ROOT/'runs/CONCEPT-010'; root.mkdir(parents=True, exist_ok=True)
    from sera_field.concept_language_study import phrases
    bank = root/'PHRASE_BANKS.json'
    if not bank.exists(): write_json(bank, phrases())
    parent = ROOT/'runs/CONCEPT-009'
    ...
    if frozen.exists():
        if json.loads(frozen.read_text()) != record: raise ValueError('Frozen campaign changed')
    else: write_json(frozen, record)
    subprocess.run([sys.executable, '-m', 'sera_field.concept_language_study'], check=True)
    roots = {'repair': root, 'unchanged': parent, 'original': ROOT/'runs/CONCEPT-008/reward',
             'echo': ROOT/'checkpoints/ECHO-007'}
    ...
    if register.exists():
        if json.loads(register.read_text()) != registration: raise ValueError('Final selections changed')
    ...
    for arm, folder in roots.items():
        for phase in ('final', 'replay'):
            subprocess.run([sys.executable, '-m', 'sera_field.concept_study', 'evaluate', '--arm', arm,
                '--training', str(folder), '--output', str(root/phase/arm), '--experiment', 'CONCEPT-010',
    ...
    decision = {'checks': checks, 'qualified': all(checks.values()),
                'all_four_replays_exact': True, 'completed_utc': utc()}
