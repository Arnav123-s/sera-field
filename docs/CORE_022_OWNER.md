...
```mermaid
flowchart LR
    A[Observed text or measured numbers] --> B[Learned source and conditional flow]
    B --> C[One coupled situation and memory state]
    C --> D[Goal-conditioned fusion alternatives]
    D --> E[Copied conditional states in the same dynamics]
    E --> F[Answer and investigation decisions]
    F --> G[Performed observation and independent grade]
    G --> C
    G --> H[Bound credit and actual weight update]
    H --> B
    C --> I[Exact protected state and resumable question]
```
